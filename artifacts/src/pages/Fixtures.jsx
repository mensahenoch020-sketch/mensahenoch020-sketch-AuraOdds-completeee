...
export default function Fixtures() {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const res = await fetch('/aura-api/fixtures');
  ...
  async function predictOne(match) {
    ...
    try {
      const res = await fetch('/aura-api/predict', {
        method: 'POST',
  ...
  return (
    <div>
      <div style={{ marginBottom: 20 }}>
        <h1 style={{ fontSize: 22, fontWeight: 700, marginBottom: 4, color: '#FFD700' }}>Fixtures Calendar</h1>
        <p style={{ color: '#777', fontSize: 13 }}>
          {allDates.length > 0
            ? `${Object.values(byDate).flat().length} matches across ${allDates.length} days`
            : 'Browse upcoming matches and get instant predictions'}
        </p>
      </div>
        ...
        {allDates.map(day => {
          ...
          return (
            <button key={day} onClick={() => setSelectedDay(day)} style={{
              background: active ? 'rgba(245,166,35,0.15)' : '#1c1c1c',
              border: `1px solid ${active ? '#F5A623' : '#222'}`,
              borderRadius: 10, padding: '8px 14px', flexShrink: 0,
              display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 2,
              cursor: 'pointer', transition: 'all 0.15s'
            }}>
              <span style={{ fontSize: 10, color: active ? '#F5A623' : '#777', fontWeight: 600 }}>{label}</span>
              <span style={{ fontSize: 12, color: active ? '#FFD700' : '#aaa', fontWeight: 500 }}>{sub}</span>
              {count > 0 && (
                <span style={{
                  background: active ? '#F5A623' : '#222', color: active ? '#000' : '#777',
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 16, flexWrap: 'wrap' }}>
        ...
        <select value={leagueFilter} onChange={e => setLeagueFilter(e.target.value)}
          style={{ fontSize: 12, flexShrink: 0, maxWidth: 180, width: 'auto' }}>
      ...
      {loading ? (
        ...
        <div style={{ textAlign: 'center', padding: '60px 20px', color: '#555' }}>
          ...
          <div style={{ fontSize: 16, fontWeight: 500, marginBottom: 6, color: '#FFD700' }}>No matches found</div>
          <div style={{ fontSize: 13 }}>{search ? 'Try different search terms' : 'No fixtures for this day'}</div>
        </div>
      ) : (
        <>
          <div style={{ fontSize: 12, color: '#777', marginBottom: 10 }}>{filtered.length} match{filtered.length !== 1 ? 'es' : ''}</div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            {filtered.map(match => {
              const key = `${match.home}_${match.away}_${match.date}`;
              return (
